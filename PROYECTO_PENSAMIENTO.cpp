...
{
int sep, sep2;
string nombresest[200], auxn, escoger, aliasest, aliasest1[200], auxalias;
...
double tiempoest[200], auxt;
    ...
    sep=0;
    for ( int i =0;i<tam;i++){
        for(int m=0; m<j[i].tamp ; m++){
            if(j[i].partidas[m].nivel>=1&& j[i].partidas[m].nivel<5 && j[i].partidas[m].puntaje > 0 && j[i].partidas[m].tiempo>0)
                {
...
for (int m = 0; m < tam; m++)
	{
		...
		cout << "* alias: " << j[m].alias << endl;
		cout << "* correo: " << j[m].correo << endl;
		for(int s=0;s<j[m].tamp;s++){
		
		cout<<" nivel "<<j[m].partidas[s].nivel<<endl;
...
if (escoger=="s"|| escoger=="S")
{
 ...
 {
//----------------------Por apellidos----------------- todas las opciones
int tampest[200], auxtamp;
if(op2==1)
{
    ...
    for ( int i =0;i<tam;i++){
        for(int m=0; m<j[i].tamp ; j++){
            if(j[i].partidas[m].nivel>=1 && j[i].partidas[m].nivel<5)
                {
                apellidossest[sep]=j[i].apellido;
                tampest[sep]=j[i].tamp;
                sep+=1;
                m=j[i].tamp+2;
                }
            }
    ...
    for ( int i =0;i<sep;i++){    //ordenamiento de apellidos ascendente
        for(int x=i;x<sep;x++)
        {
            if(tampest[i]>tampest[x]){
...
{
        for (int i = 0; i < tam; i++)
    if(apellidossest[f] == j[i].apellido)
        {
            ...
            cout << apellidossest[f] <<"      "<<"TOTAL:"<<tot1<<"    "<<"GANADAS: "<<gan1<<"    "<<(gan1/tot1)*100<<"%"<<endl;
        }
    }
}
    ...
    if(apellidossest[f] == j[i].apellido)
        {
            ...
            cout << apellidossest[f] <<"   "<<tot1<<"    "<<gan1<<"    "<<(gan1/tot1)*100<<"%"<<endl;
        }
    }
}		
